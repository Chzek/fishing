    ...
    <div class="bg-white dark:bg-slate-900 rounded-2xl p-6 sm:p-8 shadow-sm border border-slate-200/80 dark:border-slate-800 space-y-6 transition-colors">
        <div class="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-4">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h1 class="text-xl font-bold text-slate-900 dark:text-white tracking-tight">Edit Angler Profile</h1>
                    <p class="text-xs text-slate-500 dark:text-slate-400">{{ $angler->fullName }}</p>
                </div>
            </div>
            <a href="/angler/{{ $angler->id }}" class="text-xs font-semibold text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 bg-slate-100 dark:bg-slate-800 px-3 py-1.5 rounded-xl border border-slate-200 dark:border-slate-700">Return</a>
        </div>
        ...
        <form action="{{ url('/angler') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div class="space-y-1.5">
                    ...
                    <input type="text" id="firstName" name="firstName" value="{{ old('firstName', $angler->firstName) }}" required class="w-full h-11 px-3.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800 text-slate-800 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 text-sm focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500">
                </div>
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="text" id="middleName" name="middleName" value="{{ old('middleName', $angler->middleName) }}" class="w-full h-11 px-3.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800 text-slate-800 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 text-sm focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500" placeholder="Optional...">
                </div>
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="text" id="lastName" name="lastName" value="{{ old('lastName', $angler->lastName) }}" required class="w-full h-11 px-3.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800 text-slate-800 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 text-sm focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                @if(auth()->user() && auth()->user()->isAdmin())
                    <div class="space-y-1.5">
                        ...
                        <select id="user_id" name="user_id" class="w-full h-11 px-3.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800 text-slate-800 dark:text-slate-100 text-sm focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500">
                            ...
                            @foreach($users as $u)
                                <option value="{{ $u->id }}" {{ (string)old('user_id', $angler->user_id) === (string)$u->id ? 'selected' : '' }}>
                                    {{ $u->name }} ({{ $u->email }})
                                </option>
                            @endforeach
                        </select>
                ...
                @else
                    <input type="hidden" name="user_id" value="{{ $angler->user_id }}">
                    <div class="space-y-1.5">
                        <label class="block text-xs font-bold uppercase tracking-wider text-slate-700 dark:text-slate-300">Linked User Account</label>
                        <div class="h-11 px-3.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-100/70 dark:bg-slate-800/70 text-slate-600 dark:text-slate-300 text-sm flex items-center font-medium">
                            {{ $angler->user->name ?? 'Unlinked' }}
                        </div>
                    </div>
                @endif
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="date" id="birthdate" name="birthdate" value="{{ old('birthdate', $angler->birthdate ? ($angler->birthdate instanceof \DateTimeInterface ? $angler->birthdate->format('Y-m-d') : substr((string) $angler->birthdate, 0, 10)) : '') }}" class="w-full h-11 px-3.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800 text-slate-800 dark:text-slate-100 text-sm focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500">
                </div>
            </div>
            ...
            <div class="pt-4 flex items-center gap-3">
                ...
                <a href="/angler/{{ $angler->id }}" class="px-4 py-3 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 font-semibold text-sm rounded-xl border border-slate-200 dark:border-slate-700 transition-colors">Cancel</a>
            </div>
