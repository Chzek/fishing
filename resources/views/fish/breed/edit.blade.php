    ...
    <div class="bg-white dark:bg-slate-900 rounded-2xl p-6 sm:p-8 shadow-sm border border-slate-200/80 dark:border-slate-800 space-y-6">
        <div class="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-4">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h1 class="text-xl font-bold text-slate-900 dark:text-white tracking-tight">Edit Fish Species</h1>
                    <p class="text-xs text-slate-500 dark:text-slate-400">{{ $breed->name }}</p>
                </div>
            </div>
        ...
        <form action="{{ url('fish/breed') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            ...
            <div class="space-y-1.5">
                ...
                <input type="text" id="name" name="name" value="{{ old('name', $breed->name) }}" list="nameList" required class="w-full h-11 px-3.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800 text-slate-800 dark:text-slate-200 text-sm focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500">
            ...
            <div class="space-y-2 pt-2 border-t border-slate-100 dark:border-slate-800">
                ...
                <div class="flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-800/60 rounded-2xl border border-slate-200/60 dark:border-slate-700/60">
                    ...
                    <div class="flex-1">
                        ...
                        <span class="text-[10px] text-slate-400 dark:text-slate-500 block mt-1">Square species avatar used in table rows & lists.</span>
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-2">
                ...
                <div class="flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-800/60 rounded-2xl border border-slate-200/60 dark:border-slate-700/60">
                    <div class="w-20 h-14 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 p-1 flex items-center justify-center shrink-0 overflow-hidden">
                        @if($breed->imageUrl)
                    ...
                    <div class="flex-1 w-full">
                        <input type="file" id="image" name="image" class="w-full text-xs text-slate-500 dark:text-slate-400 file:mr-3 file:py-1.5 file:px-3 file:rounded-xl file:border-0 file:text-xs file:font-semibold file:bg-slate-200/70 dark:file:bg-slate-700 file:text-slate-700 dark:file:text-slate-300 hover:file:bg-slate-300 dark:hover:file:bg-slate-600 transition-all cursor-pointer">
                        <span class="text-[10px] text-slate-400 dark:text-slate-500 block mt-1">Wide side-profile illustration used on species dossier page.</span>
                    </div>
                </div>
            </div>
