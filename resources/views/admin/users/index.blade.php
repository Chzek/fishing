...
<div class="space-y-6">
    ...
    @if(!empty($unreadNotifications) && $unreadNotifications->count() > 0)
        <div class="bg-amber-50/80 dark:bg-amber-950/40 rounded-2xl p-4 sm:p-5 border border-amber-200/80 dark:border-amber-800/60 shadow-sm flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div class="flex items-center gap-3.5">
                ...
                <div>
                    <h3 class="text-sm font-bold text-slate-900 dark:text-white tracking-tight flex items-center gap-2">
                        ...
                        <span class="px-2 py-0.5 text-[10px] font-bold rounded-full bg-amber-200/70 dark:bg-amber-900/60 text-amber-900 dark:text-amber-200 border border-amber-300 dark:border-amber-700 font-mono">New</span>
                    </h3>
                    <p class="text-xs text-slate-600 dark:text-slate-300 mt-0.5">Pair newly registered accounts with their corresponding Angler profile in the table below.</p>
                </div>
            </div>
            <form action="{{ route('admin.notifications.mark_read') }}" method="POST" class="shrink-0">
                @csrf
                <button type="submit" class="px-3.5 py-2 bg-white dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 font-semibold text-xs rounded-xl border border-slate-200/80 dark:border-slate-700 shadow-sm transition-colors flex items-center gap-1.5 cursor-pointer">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
        ...
        <div class="bg-white dark:bg-slate-900 rounded-2xl p-5 border border-slate-200/80 dark:border-slate-800 shadow-sm space-y-3 transition-colors">
            <div class="flex items-center gap-2 border-b border-slate-100 dark:border-slate-800 pb-2.5">
                <x-lucide-mail-plus class="w-4 h-4 text-teal-600 dark:text-teal-400" />
                <h3 class="font-bold text-slate-900 dark:text-white text-xs uppercase tracking-wider">Email Signed Invitation (Home Mode)</h3>
            </div>
            <p class="text-xs text-slate-500 dark:text-slate-400">Generates a secure signed registration URL valid for 7 days to email an angler.</p>
            <form action="{{ route('admin.users.invite') }}" method="POST" class="space-y-3">
                @csrf
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    <input type="text" name="name" required placeholder="Angler Full Name" class="h-9 px-3 rounded-xl border border-slate-200 dark:border-slate-700 text-xs bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500">
                    <input type="email" name="email" required placeholder="Angler Email Address" class="h-9 px-3 rounded-xl border border-slate-200 dark:border-slate-700 text-xs bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500">
                </div>
        ...
        <div class="bg-white dark:bg-slate-900 rounded-2xl p-5 border border-slate-200/80 dark:border-slate-800 shadow-sm space-y-3 transition-colors">
            <div class="flex items-center gap-2 border-b border-slate-100 dark:border-slate-800 pb-2.5">
                <x-lucide-user-plus class="w-4 h-4 text-amber-600 dark:text-amber-400" />
                <h3 class="font-bold text-slate-900 dark:text-white text-xs uppercase tracking-wider">Canada Offline Quick-Add (Field Mode)</h3>
            </div>
            <p class="text-xs text-slate-500 dark:text-slate-400">Create an Angler account locally without internet. Syncs upstream to NAS when returning home.</p>
            <form action="{{ route('admin.users.quick-add') }}" method="POST" class="space-y-3">
                @csrf
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-2">
                    <input type="text" name="name" required placeholder="Angler Name" class="h-9 px-3 rounded-xl border border-slate-200 dark:border-slate-700 text-xs bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500">
                    <input type="email" name="email" required placeholder="Email" class="h-9 px-3 rounded-xl border border-slate-200 dark:border-slate-700 text-xs bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500">
                    <input type="password" name="password" required placeholder="Temp Password" class="h-9 px-3 rounded-xl border border-slate-200 dark:border-slate-700 text-xs bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500">
                </div>
    ...
    <div class="bg-white dark:bg-slate-900 rounded-2xl p-6 shadow-sm border border-slate-200/80 dark:border-slate-800 space-y-4 transition-colors">
        <div class="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
            <h2 class="font-bold text-slate-900 dark:text-white text-sm flex items-center gap-2">
                <x-lucide-user-check class="w-4 h-4 text-teal-600 dark:text-teal-400" /> Registered User Accounts
            </h2>
            <span class="text-xs text-slate-500 dark:text-slate-400 font-mono">{{ count($users) }} Account(s)</span>
        </div>
        ...
        @livewire('components.generic-data-table', [
            ...
            'columns' => [
                ...
                ['key' => 'angler.lastName', 'label' => 'Associated Angler', 'type' => 'angler_name', 'sortable' => true, 'sortKey' => 'angler'],
                ['key' => 'assignment', 'label' => 'Assign Angler', 'type' => 'admin_user_assignment', 'align' => 'right'],
            ],
