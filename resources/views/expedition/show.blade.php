...
<div class="space-y-6">
    ...
    >
        ...
        <x-slot:actions>
            @if(view()->exists('expedition.edit'))
                <a href="/expedition/{{ $expedition->id }}/edit" class="p-2 rounded-xl text-slate-300 hover:text-white bg-slate-800 hover:bg-slate-700 transition-colors border border-slate-700" title="Edit Expedition">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <!-- 👑 Lunker Legend -->
        <div class="bg-gradient-to-br from-amber-500/10 via-amber-500/5 to-transparent bg-white dark:bg-slate-900 p-5 rounded-2xl border border-amber-200 dark:border-amber-800/40 shadow-sm relative overflow-hidden space-y-2">
            ...
            <div class="flex items-center justify-between relative z-10">
                ...
                <x-badge variant="amber" size="xs" label="Trophy" :fontMono="true" />
            </div>

            @if($lunker)
                <div class="space-y-1 pt-1 relative z-10">
                    <div class="flex items-baseline gap-1.5">
                        <span class="text-3xl font-black text-slate-900 dark:text-white font-mono">{{ number_format($lunker->length, 1) }}</span>
                        <span class="text-xs font-bold text-slate-500 dark:text-slate-400">inches</span>
                    </div>
                    <div class="text-xs font-bold text-teal-700 dark:text-teal-400">{{ $lunker->fishBreed->name ?? 'Fish' }}</div>
                    <div class="flex items-center gap-2 pt-1 border-t border-amber-100/80 dark:border-amber-900/40">
                        @if($lunker->angler)
                            <x-anglerAvatar :angler="$lunker->angler" size="xs" />
                            <span class="text-xs font-semibold text-slate-800 dark:text-slate-200">{{ $lunker->angler->fullName }}</span>
                        @endif
                    </div>
                </div>
            @else
                <div class="text-xs text-slate-400 dark:text-slate-500 py-3 italic relative z-10">No catches logged yet.</div>
            @endif
        </div>
        ...
        <div class="bg-gradient-to-br from-sky-500/10 via-sky-500/5 to-transparent bg-white dark:bg-slate-900 p-5 rounded-2xl border border-sky-200 dark:border-sky-800/40 shadow-sm space-y-2 relative overflow-hidden">
            ...
            @if($heavyweight)
                <div class="space-y-1 pt-1 relative z-10">
                    <div class="flex items-baseline gap-1.5">
                        <span class="text-3xl font-black text-slate-900 dark:text-white font-mono">{{ number_format($heavyweight->weight, 1) }}</span>
                        <span class="text-xs font-bold text-slate-500 dark:text-slate-400">lbs.</span>
                    </div>
                    <div class="text-xs font-bold text-teal-700 dark:text-teal-400">{{ $heavyweight->fishBreed->name ?? 'Fish' }}</div>
                    <div class="flex items-center gap-2 pt-1 border-t border-sky-100/80 dark:border-sky-900/40">
                        @if($heavyweight->angler)
                            <x-anglerAvatar :angler="$heavyweight->angler" size="xs" />
                            <span class="text-xs font-semibold text-slate-800 dark:text-slate-200">{{ $heavyweight->angler->fullName }}</span>
                        @endif
                    </div>
                </div>
            @else
                <div class="text-xs text-slate-400 dark:text-slate-500 py-3 italic relative z-10">No weight entries logged.</div>
            @endif
        </div>

        <!-- ⚡ Top Rod MVP -->
        <div class="bg-gradient-to-br from-emerald-500/10 via-emerald-500/5 to-transparent bg-white dark:bg-slate-900 p-5 rounded-2xl border border-emerald-200 dark:border-emerald-800/40 shadow-sm space-y-2 relative overflow-hidden">
            <x-watermarkTopRod />
            <div class="flex items-center justify-between relative z-10">
                <span class="text-[10px] font-bold uppercase tracking-wider text-emerald-800 dark:text-emerald-400 flex items-center gap-1">
                    ⚡ Top Rod MVP
                </span>
                <x-badge variant="emerald" size="xs" label="Most Fish" :fontMono="true" />
            </div>

            @if($topRod && $topRod->angler)
                <div class="space-y-1 pt-1 relative z-10">
                    <div class="flex items-baseline gap-1.5">
                        <span class="text-3xl font-black text-slate-900 dark:text-white font-mono">{{ $topRod->catch_count }}</span>
                        <span class="text-xs font-bold text-slate-500 dark:text-slate-400">catches</span>
                    </div>
                    <div class="text-xs font-semibold text-slate-600 dark:text-slate-400 font-mono">{{ number_format($topRod->total_length, 1) }} total inches</div>
                    <div class="flex items-center gap-2 pt-1 border-t border-emerald-100/80 dark:border-emerald-900/40">
                        <x-anglerAvatar :angler="$topRod->angler" size="xs" />
                        <span class="text-xs font-bold text-slate-900 dark:text-white">{{ $topRod->angler->fullName }}</span>
                    </div>
                </div>
            @else
                <div class="text-xs text-slate-400 dark:text-slate-500 py-3 italic relative z-10">No catches logged.</div>
            @endif
        </div>

        <!-- 🎣 Hot Lure -->
        <div class="bg-gradient-to-br from-purple-500/10 via-purple-500/5 to-transparent bg-white dark:bg-slate-900 p-5 rounded-2xl border border-purple-200 dark:border-purple-800/40 shadow-sm space-y-2 relative overflow-hidden">
            ...
            <div class="flex items-center justify-between relative z-10">
                <span class="text-[10px] font-bold uppercase tracking-wider text-purple-800 dark:text-purple-400 flex items-center gap-1">
                    🎣 MVP Hot Lure
                </span>
                <x-badge variant="purple" size="xs" label="Top Bait" :fontMono="true" />
            </div>

            @if($hotLure && $hotLure->lure)
                <div class="space-y-1 pt-1 relative z-10">
                    <div class="text-lg font-black text-slate-900 dark:text-white line-clamp-1" title="{{ $hotLure->lure->displayName }}">
                        {{ $hotLure->lure->name }}
                    </div>
                    <div class="text-xs font-semibold text-slate-500 dark:text-slate-400">{{ $hotLure->lure->color }} {{ $hotLure->lure->size }}</div>
                    <div class="pt-1 border-t border-purple-100/80 dark:border-purple-900/40 flex items-center justify-between text-xs">
                        <span class="text-slate-500 dark:text-slate-400">Catches:</span>
                        <strong class="font-mono text-purple-700 dark:text-purple-400">{{ $hotLure->catch_count }} fish</strong>
                    </div>
                </div>
            @else
                <div class="text-xs text-slate-400 dark:text-slate-500 py-3 italic relative z-10">No lure data recorded.</div>
            @endif
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <x-card title="Daily Catch Cadence" icon="lucide-trending-up" badge="Fish Per Day" badgeVariant="slate">
            <div>
                @if(count($dailyCadence) > 0)
                    ...
                    @if($peakDay)
                        <div class="mb-3 flex items-center justify-between bg-teal-50/80 dark:bg-teal-950/40 border border-teal-200/60 dark:border-teal-800 rounded-xl px-3 py-1.5 text-xs text-teal-800 dark:text-teal-300 font-medium">
                            <span class="flex items-center gap-1.5">
                                <span>🔥 Peak Day:</span>
                                <strong class="font-bold">{{ date('M j', strtotime($peakDay->caught)) }}</strong>
                            </span>
                            <span class="font-mono font-bold text-teal-700 dark:text-teal-400">{{ $peakDay->count }} fish</span>
                        </div>
                    @endif

                    <div class="pt-1 space-y-2">
                        <div class="h-60 flex items-end justify-between gap-2 px-1 pb-2 border-b border-slate-100 dark:border-slate-800">
                            @foreach($dailyCadence as $day)
                                ...
                                <div class="flex-1 h-full flex flex-col justify-end items-center gap-1.5 group relative">
                                    <div class="w-full bg-slate-100 dark:bg-slate-800 hover:bg-teal-50 dark:hover:bg-teal-950/40 rounded-t-xl transition-all flex items-end justify-center overflow-hidden" style="height: 100%;">
                                    ...
                                    <span class="text-[9px] font-mono text-slate-600 dark:text-slate-400 font-bold truncate w-full text-center">{{ date('M j', strtotime($day->caught)) }}</span>
                                </div>
                            @endforeach
                        </div>
                    </div>
                @else
                    <div class="text-center py-10 text-slate-400 dark:text-slate-500 text-xs italic">
            ...
            @if(count($dailyCadence) > 0)
                <x-slot:footer>
                    <div class="grid grid-cols-2 gap-2 text-xs">
                        <div class="p-2.5 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200/60 dark:border-slate-800 space-y-0.5">
                            <span class="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider block">Daily Average</span>
                            <strong class="text-sm font-black text-slate-900 dark:text-white font-mono block">{{ $dailyAvgCatches }} <span class="text-[10px] font-normal text-slate-500">fish/day</span></strong>
                        </div>
                        <div class="p-2.5 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200/60 dark:border-slate-800 space-y-0.5">
                            <span class="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider block">Active Days</span>
                            <strong class="text-sm font-black text-slate-900 dark:text-white font-mono block">{{ $daysFishedCount }} <span class="text-[10px] font-normal text-slate-500">of {{ $totalTripDays }} d</span></strong>
                        </div>
                    </div>
                </x-slot:footer>
            @endif
        ...
        <x-card title="Species Breakdown" icon="lucide-pie-chart" :badge="count($speciesDistribution) . ' Species'" badgeVariant="slate">
            @if(count($speciesDistribution) > 0)
                ...
                <div class="flex flex-col items-center gap-4 py-1">
                    <!-- Donut Pie Chart -->
                    <div class="relative w-36 h-36 rounded-full shadow-md border-4 border-white dark:border-slate-800 shrink-0 transition-transform hover:scale-105" style="background: conic-gradient({{ $conicStyle }});">
                        <div class="absolute inset-4 rounded-full bg-white dark:bg-slate-900 flex flex-col items-center justify-center border border-slate-100 dark:border-slate-800 shadow-inner">
                            <span class="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Species</span>
                            <span class="text-xl font-black text-slate-900 dark:text-white font-mono leading-none my-0.5">{{ count($speciesDistribution) }}</span>
                            <span class="text-[10px] text-teal-600 dark:text-teal-400 font-bold font-mono">{{ $totalRecords }} fish</span>
                        </div>
                    </div>

                    <!-- Legend Breakdown -->
                    <div class="w-full space-y-2 pt-2 border-t border-slate-100 dark:border-slate-800">
                        @foreach($speciesDistribution as $idx => $sp)
                            @php
                            ...
                            <div class="flex items-center justify-between text-xs px-0.5">
                                <div class="flex items-center gap-2 min-w-0">
                                    ...
                                    <span class="font-bold text-slate-800 dark:text-slate-200 truncate">{{ $sp->fishBreed->name ?? 'Unknown Species' }}</span>
                                </div>
                                <div class="flex items-center gap-2 font-mono shrink-0 pl-1">
                                    <span class="text-slate-500 dark:text-slate-400 text-[11px]">{{ $sp->count }} caught</span>
                                    <strong class="text-slate-900 dark:text-white w-9 text-right">{{ $pct }}%</strong>
                                </div>
                            </div>
                        @endforeach
                    </div>
            ...
            @if(count($speciesDistribution) > 0)
                ...
                <x-slot:footer>
                    <div class="grid grid-cols-2 gap-2 text-xs">
                        <div class="p-2.5 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200/60 dark:border-slate-800 space-y-0.5">
                            <span class="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider block">Dominant Species</span>
                            <strong class="text-xs font-black text-slate-900 dark:text-white truncate block">{{ $topSpecies->fishBreed->name ?? 'None' }}</strong>
                            <span class="text-[10px] text-teal-600 dark:text-teal-400 font-bold font-mono block">{{ round(($topSpecies->count / $totalRecords) * 100) }}% share</span>
                        </div>
                        <div class="p-2.5 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200/60 dark:border-slate-800 space-y-0.5">
                            <span class="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider block">Release Rate</span>
                            <strong class="text-sm font-black text-slate-900 dark:text-white font-mono block">{{ $releaseRate }}%</strong>
                            <span class="text-[10px] text-slate-500 dark:text-slate-400 font-mono block">{{ $releasedCount }} released</span>
                        </div>
                    </div>
                </x-slot:footer>
            @endif
                ...
                @forelse($crewLeaderboard as $idx => $cl)
                    <div class="py-2.5 flex items-center justify-between text-xs">
                        ...
                        <div class="text-right font-mono shrink-0 pl-1">
                            <strong class="text-sm font-black text-slate-900 dark:text-white block">{{ $cl->total_catches }}</strong>
    ...
    <x-card title="Expedition Catch & Visited Waterbodies Map" icon="lucide-map-pin">
        <x-slot:actions>
            <span class="text-slate-500 dark:text-slate-400 font-mono text-xs font-semibold">
                {{ count($visitedLakes) }} Visited Lake(s) &bull; {{ count($recordsWithGps) }} Catch Pinpoint(s)
            </span>
        </x-slot:actions>

        <div id="expedition-gps-map" class="w-full h-80 rounded-xl border border-slate-200 dark:border-slate-800 overflow-hidden bg-slate-100 dark:bg-slate-900"></div>
    </x-card>

    <!-- Posts Journal Stream Column -->
    <x-card title="Trip Journal & Posts" icon="lucide-message-square">
        ...
        @if(count($expedition->posts) > 0)
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                @foreach($expedition->posts as $post)
                    <div class="p-4 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200/60 dark:border-slate-800 flex items-start gap-3">
                        <x-anglerAvatar :angler="$post->creator" size="md" />
                        <div class="space-y-1 flex-1">
                            <div class="flex items-center justify-between text-xs">
                                <span class="font-bold text-slate-900 dark:text-white">{{ $post->creator->full_name }}</span>
                                <span class="text-slate-400 font-mono text-[11px]">{{ $post->date }}</span>
                            </div>
                            <blockquote class="text-xs text-slate-700 dark:text-slate-300 italic border-l-2 border-teal-500 pl-2">
                                "{{ $post->description }}"
                            </blockquote>
                        </div>
                    </div>
    ...
    <x-card title="Trip Scrapbook & Photo Gallery" icon="lucide-camera" subtitle="Shared memories, scenic shots, and brag board captures">
        ...
        <div id="expedition-upload-card" class="hidden bg-slate-50 dark:bg-slate-800/60 border border-slate-200/80 dark:border-slate-800 rounded-2xl p-5 space-y-4">
            <form action="{{ route('photos.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                ...
                <div class="space-y-1">
                    <label for="exp-photo-caption" class="block text-xs font-bold uppercase tracking-wider text-slate-700 dark:text-slate-300">Optional Caption / Tagline</label>
                    <input type="text" id="exp-photo-caption" name="caption" placeholder="e.g., Evening topwater action on the north bay" class="w-full h-10 px-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-xs text-slate-800 dark:text-white focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500">
                </div>

                <div class="flex items-center justify-end gap-2 pt-2">
                    <button type="button" onclick="document.getElementById('expedition-upload-card').classList.add('hidden')" class="px-4 py-2 bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-200 text-xs font-semibold rounded-xl">Cancel</button>
        ...
        @if($expedition->photos && $expedition->photos->count() > 0)
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
                @foreach($expedition->photos as $photo)
                    <div class="relative group aspect-4/3 rounded-2xl overflow-hidden border border-slate-200 dark:border-slate-800 shadow-xs bg-slate-900">
        ...
        @else
            <div class="text-center py-10 border-2 border-dashed border-slate-100 dark:border-slate-800 rounded-2xl space-y-2">
                ...
                <p class="text-xs text-slate-500 dark:text-slate-400 font-medium">No trip photos uploaded yet.</p>
    ...
    @if(count($records) > 0)
        <x-card title="Expedition Catches Log" icon="lucide-fish">
