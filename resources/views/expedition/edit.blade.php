    ...
    <div class="bg-white rounded-2xl p-6 sm:p-8 shadow-sm border border-slate-200/80 space-y-6">
        ...
        <form action="{{ url('/expedition') }}" method="POST" class="space-y-4">
            ...
            <div class="space-y-1.5">
                ...
                <input type="text" id="description" name="description" value="{{ old('description', $expedition->description) }}" required class="w-full h-11 px-3.5 rounded-xl border border-slate-200 bg-slate-50/50 text-slate-800 text-sm focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500">
            </div>
            ...
            <div class="flex items-center gap-3 pt-4">
                ...
                <a href="/expedition/{{ $expedition->id }}" class="px-5 py-3 bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold text-sm rounded-xl border border-slate-200 transition-colors">Cancel</a>
            </div>
