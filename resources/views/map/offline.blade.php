    ...
    <div class="bg-white rounded-2xl p-6 shadow-sm border border-slate-200/80 space-y-6">
            ...
            <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                <div class="space-y-2">
                    ...
                    <p class="text-xs text-slate-600">
                        <strong>Coverage:</strong> Highway 17 Corridor (Lat 47.7°N to 48.7°N, Lng -85.5°W to -84.3°W) — Wawa Lake, Hawk Lake, Catfish Lake, Magpie River, Dubreuilville & White River waterbodies.
                    </p>
                    <div class="flex flex-wrap gap-1.5 pt-1">
                        ...
                        <span class="bg-sky-100 text-sky-800 text-[11px] font-semibold px-2.5 py-0.5 rounded-md border border-sky-200">🌊 Bathymetry & Contours</span>
                        <span class="bg-slate-200 text-slate-800 text-[11px] font-semibold px-2.5 py-0.5 rounded-md">🗺️ Topographic Hydrography</span>
                    </div>
                </div>
            </div>

            <!-- Download Action Buttons -->
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-2 border-t border-slate-200/80">
                <button id="btn-download-bathy" onclick="downloadWawaRegion('bathy')" class="py-3 px-4 bg-teal-600 hover:bg-teal-500 text-white font-bold text-xs rounded-xl shadow transition-all flex items-center justify-center gap-2 cursor-pointer">
                    <x-lucide-download class="w-4 h-4 text-teal-200" />
                    <span>🌊 Bathymetry Pack (~55 MB)</span>
                </button>
                <button id="btn-download-topo" onclick="downloadWawaRegion('topo')" class="py-3 px-4 bg-slate-800 hover:bg-slate-700 text-slate-100 font-bold text-xs rounded-xl border border-slate-700 transition-all flex items-center justify-center gap-2 cursor-pointer">
                    <x-lucide-download class="w-4 h-4 text-slate-300" />
                    <span>🗺️ Topo Pack (~55 MB)</span>
                </button>
                <button id="btn-download-all" onclick="downloadWawaRegion('all')" class="py-3 px-4 bg-gradient-to-r from-teal-700 to-indigo-700 hover:from-teal-600 hover:to-indigo-600 text-white font-bold text-xs rounded-xl shadow transition-all flex items-center justify-center gap-2 cursor-pointer">
                    <x-lucide-layers class="w-4 h-4 text-teal-200" />
                    <span>📦 Download Both (~110 MB)</span>
                </button>
            </div>
        </div>
        ...
        <div class="bg-slate-900 text-slate-200 rounded-2xl p-5 border border-slate-800 space-y-4">
            <div class="flex items-center justify-between">
                ...
                <button onclick="clearTileCache()" class="text-xs font-semibold text-rose-400 hover:text-rose-300 hover:bg-rose-950/40 px-3 py-1 rounded-lg border border-rose-500/30 transition-colors flex items-center gap-1 cursor-pointer">
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // OpenTopoMap Bathymetry & Relief Contours Base Layer (Default active)
        bathyBaseLayer = L.tileLayer('https://tile.opentopomap.org/{z}/{x}/{y}.png', {
            maxZoom: 16,
        ...
        }, null, { position: 'bottomleft' }).addTo(previewMap);
    ...
    async function downloadWawaRegion(packType = 'bathy') {
        if (!('caches' in window)) {
            alert('CacheStorage is not supported on this browser.');
            return;
        }

        const btnBathy = document.getElementById('btn-download-bathy');
        const btnTopo = document.getElementById('btn-download-topo');
        ...
        if (btnBathy) btnBathy.disabled = true;
        if (btnTopo) btnTopo.disabled = true;
        if (btnAll) btnAll.disabled = true;

        statusMsg.innerText = '⏳ Calculating map tile coordinates for region...';
        ...
        for (let z = minZoom; z <= maxZoom; z++) {
            ...
            for (let x = minTile.x; x <= maxTile.x; x++) {
                for (let y = minTile.y; y <= maxTile.y; y++) {
                    if (packType === 'bathy' || packType === 'all') {
                        urls.push(`https://tile.opentopomap.org/${z}/${x}/${y}.png`);
                    }
                    if (packType === 'topo' || packType === 'all') {
        ...
        if (btnBathy) btnBathy.disabled = false;
        if (btnTopo) btnTopo.disabled = false;
        if (btnAll) btnAll.disabled = false;

        const packLabel = packType === 'bathy' ? 'Bathymetry & Contours' : (packType === 'topo' ? 'Topo / Hydrography' : 'Complete');
        statusMsg.innerText = `🎉 Successfully pre-cached ${total.toLocaleString()} tiles (${packLabel} Pack) for offline boat use!`;
        alert(`🎉 ${packLabel} Map Pack successfully downloaded for offline use!`);
    }
